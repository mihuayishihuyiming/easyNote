...
- **统计**：左上角显示**已记账天数**（按记账日期去重），点击进入**我的记账日记**（已记账天数/笔数、大手大脚次数、后悔次数），日记可一键生成海报（含当前时间与「随随随记」水印）**保存到相册或分享给好友**；按月查看**支出条形统计图**，可切换历史月份并一键回到本月；支出/收入分类明细点击可下钻到该月该分类的逐笔记录（含时间与标红提醒）
